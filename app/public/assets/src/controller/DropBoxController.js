class DropBoxController {
    ...
    this.getSelection().forEach((li) => {
      ...
      promises.push(
        new Promise((resolve, reject) => {
          this.currentFold.join("/");
          file.name;

          if (file.type === "folder") {
            this.removeFolderTask(this.currentFold.join("/"), file.name).then(
              () => {
                resolve({
                  fields: {
                    key,
            ...
            this.removeFile(this.currentFold.join("/"), file.name).then(() => {
              resolve({
                fields: {
                  key,
                },
  ...
  initEvents() {
    ...
    this.listFileEL.addEventListener("selectionchange", (e) => {
      switch (this.getSelection().lenght) {
        case 0:
          this.btnDelete.style.disabled = "none";
          this.btnRename.style.disabled = "none";
          break;
        case 1:
          this.btnDelete.style.disabled = "block";
          this.btnRename.style.disabled = "block";
          break;
        default:
          this.btnDelete.style.disabled = "block";
          this.btnRename.style.disabled = "none";
  ...
  getFirebaseRef(path) {
    if (!path) path = this.currentFold.join("/");
  ...
  uploadProgress(event, file) {
    ...
    const timeleft = (100 / percent) * timespent - timespent;
  ...
  formatTimeToHuman(duration) {
    ...
    if (hours > 0) {
      ...
      return "tempo estimado indefinido";
    }
  }
  ...
  readFiles() {
    this.lastFolder = this.currentFold.join("/");
  ...
  renderNav() {
    ...
    for (let i = 0; i < this.currentFold.length; i++) {
      let folderName = this.currentFold[i];
      let span = document.createElement("span");

      path.push(folderName);

      if (i + 1 === this.currentFold.length) {
        span.innerHTML = folderName;
      } else {
        span.className = "breadcrumb-segment__wrapper";
        span.innerHTML = `
                        <span
                          class="ue-effect-container uee-BreadCrumbSegment-link-0"
                        >
                          <a
                            href="#" data-path="${path.join("/")}"
    ...
    this.navEL.querySelectorAll("a").forEach((a) => {
      a.addEventListener("click", (e) => {
        ...
        this.currentFold = a.dataset.path.split("/");
  ...
  initEventsLi(li) {
    li.addEventListener("dblclick", (e) => {
      ...
      switch (file.type) {
        case "folder":
          this.currentFold.push(file.name);
          this.openFolder();
          break;
        default:
    ...
    li.addEventListener("click", (e) => {
      if (e.shiftKey) {
        ...
        if (firstLi) {
          let indexStart;
          let indexEnd;
          let lis = li.parentElements.childNodes;
